...
class Asteroids extends Game {
	...
	//width and height *for best results keep w = 800 and h = 600
	static int w = 800;
	static int h = 600;
	...
	public void paint(Graphics brush) {
		...
		// the stars in a starry kind of way
		...
		if (start < 1) {
			...
			brush.setFont(new Font("Monospaced", Font.BOLD, 10));
			brush.drawString(
					"|||||||||    |||||||||   |||||||||||    |||||||||    |||||||||     |||||||||  |||||||||||   ||||||||",
					100, 130);
			brush.drawString(
			...
			brush.setColor(Color.white);
			brush.setFont(new Font("Dialog", Font.PLAIN, 12));
			brush.drawString("Avoid collision...", 330, 290);
			brush.drawString("And shoot your way to safety...", 330, 310);
			if (delay%100 < 10 ) {
				brush.setColor(Color.gray);
			} else if(delay%100 > 20  && delay%100 < 30){
				brush.setColor(Color.DARK_GRAY);
			}
			brush.drawString("Press Any Key to begin", 330, 330);

			// if press other key exit the start
			if ((ship.otherKey || ship.upKey || ship.downKey || ship.leftKey || ship.rightKey || ship.space
					|| ship.shift || ship.one || ship.two || ship.three || ship.four) && delay > 100) {
				start = 1;
			}
